...
def extract_video_clips(args):
    ...
    for dataType in ['val']:
        ...
        for entity in tqdm.tqdm(entityList, total = len(entityList)):
            ...
            #print('video file path ', videoFile)
            V = cv2.VideoCapture(videoFile)
            #print('fps ', V.get(cv2.CAP_PROP_FPS))
            #print('num frames ', V.get(cv2.CAP_PROP_FRAME_COUNT))
            ...
            for _, row in insData.iterrows():
                ...
                #print('image file name ', imageFilename, row['frame_timestamp'] * 1e3)
